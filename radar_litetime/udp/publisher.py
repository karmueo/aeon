...
class MulticastPublisher:
    ...
    def __init__(self, group: str, port: int, iface: str = "0.0.0.0", ttl: int = 1):
        """初始化发布器。
    ...
    def open(self) -> "MulticastPublisher":
        """打开组播 socket。
    ...
    def _build_packet(self, item: dict) -> bytes:
        """构建单个分类结果报文。
        ...
        base_day = int(
            item.get("base_day", now.tm_year * 10000 + now.tm_mon * 100 + now.tm_mday)
        ) & 0xFFFFFFFF
